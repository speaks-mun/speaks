...
// ✅ Now you can safely log the schema after it's imported
console.log("loginSchema after import:", loginSchema);
console.log("loginSchema type:", typeof loginSchema);
console.log("loginSchema._def:", loginSchema._def);
...
export default function LoginPage() {
	...
	const { execute: handleLogin, isLoading } = useApiAction(
		async (data: LoginFormData) => {
			...
			console.log("Using loginSchema:", loginSchema);
			...
			<Card className="w-full max-w-md border-border-divider bg-card-background">
				<CardHeader className="space-y-1">
					...
					<CardDescription className="text-body-text">
						Sign in to your Speaks account to continue
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					...
					<div className="relative">
						...
						<div className="relative flex justify-center text-xs uppercase">
							<span className="bg-card-background px-2 text-body-text">
								Or continue with
							</span>
						</div>
					</div>
					...
					<div className="space-y-2">
						<Button
							disabled
							variant="outline"
							className="w-full border-border-divider bg-transparent"
						>
							Sign in with Google
						</Button>
						<Button
