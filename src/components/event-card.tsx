  ...
  return (
    <Card
      className={cn(
        ...
        event.is_full && "ring-2 ring-yellow-500/20",
      )}
    >
      {/* Header Background */}
      <div className="relative h-48 overflow-hidden bg-gray-700">
        ...
        <Button
          variant="ghost"
          size="icon"
          className="absolute top-3 right-3 h-8 w-8 rounded-full bg-white/20 backdrop-blur-sm hover:bg-white/30"
          onClick={(e) => {
            e.preventDefault()
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <Button
            ...
            className={cn(
              ...
              event.is_past && "opacity-50 cursor-not-allowed",
              event.is_full && "bg-yellow-500 hover:bg-yellow-600",
            )}
            disabled={event.is_past}
            asChild={!event.is_past}
          >
            {event.is_past ? (
              <span>Ended</span>
            ) : event.is_full ? (
